...
Industry::Industry(const Settlement *settlement_, const Product *product_) : settlement(settlement_), product(product_)
{
    ...
    settlement->population->workforce += workforce;
...
{
    income = market->transaction;
    //std::cout<<market->order<<" "<<market->demand<<"\n";
...
void Industry::UpdateWorkforce()
{
    ...
    if(rentability > 0.0f)
    {
        ...
        if(utility::RollDice(increaseChance) && settlement->population->HasFreeWorkers())
        {
            workforce++;
            settlement->population->workforce++;
        }
    }
    else
    {
        auto decreaseChance = pow(-rentability, 1.0f);
        if(utility::RollDice(decreaseChance))
        {
            workforce--;
            settlement->population->workforce--;
        }
